...
export class FireflyClient {
  ...
  private auth: FireflyAuth | null = null;
  ...
  private async authenticate(): Promise<FireflyAuth> {
    // Reuse token until 60 seconds before expiry
    if (this.auth && Date.now() < this.auth.expiresAt - 60_000) {
      return this.auth;
    }
    ...
    if (!response.ok) {
      throw new Error(
        `Adobe Auth Error (${response.status}): ${data.error_description || data.error || JSON.stringify(data)}`
      );
    }
  ...
  private async request<T>(
    ...
    const data: any = await response.json();
    if (!response.ok) {
      const error = data as FireflyError;
      throw new Error(
        `Firefly API Error (${response.status}): ${error.error_description || error.error || JSON.stringify(data)}`
      );
    }
  ...
  async uploadImage(
    ...
    if (!response.ok) {
      throw new Error(
        `Firefly Upload Error (${response.status}): ${JSON.stringify(data)}`
      );
    }
  ...
  async generateImageAsync(
  ...
  ): Promise<GenerateAsyncJobResponse> {
    ...
    return this.request<GenerateAsyncJobResponse>('/v3/images/generate-async', {
      method: 'POST',
  ...
  async generateImageSync(
    ...
    return this.request<GenerateImageResponse>('/v3/images/generate', {
      method: 'POST',
      body: JSON.stringify(body),
    });
  }
  ...
  async generativeFillAsync(
    request: GenerativeFillRequest
  ): Promise<GenerateAsyncJobResponse> {
    return this.request<GenerateAsyncJobResponse>('/v3/images/fill-async', {
      method: 'POST',
  ...
  async expandImageAsync(
    ...
    return this.request<GenerateAsyncJobResponse>('/v3/images/expand-async', {
      method: 'POST',
      body: JSON.stringify(request),
    });
  }
  ...
  async generateSimilarAsync(
    ...
    if (numVariations) body.numVariations = numVariations;

    return this.request<GenerateAsyncJobResponse>(
  ...
  async objectCompositeAsync(
    request: Record<string, any>
  ): Promise<GenerateAsyncJobResponse> {
    return this.request<GenerateAsyncJobResponse>(
      '/v3/images/object-composite-async',
      {
        method: 'POST',
        body: JSON.stringify(request),
      }
  ...
  async pollJob(
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      if (!response.ok) {
        throw new Error(
          `Poll Error (${response.status}): ${JSON.stringify(data)}`
        );
      }
  ...
  async cancelJob(cancelUrl: string): Promise<void> {
    const auth = await this.authenticate();
